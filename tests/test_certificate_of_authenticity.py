...
class TestCertificateOfAuthenticity(object):
    ...
    @classmethod
    def setup_class(cls):
        cls.code = load_serpent('serpent/certificate_of_authenticity.se')
    ...
    def test_authenticity(self):
        data = [self.ARTIST.address, self.ARTWORK_HASH]
        response = self.sim.tx(self.BEHOLDER, self.contract, 0, data)
        assert response[0] == 1

    def test_authenticity_bad_artwork_hash(self):
        data = [self.ARTIST.address, self.ARTWORK_HASH + 1]
        response = self.sim.tx(self.BEHOLDER, self.contract, 0, data)
        assert response[0] == 0
